...
# 读取 JSON 文件
with open('data/oa/08.json', 'r', encoding='utf-8') as file:
    data = json.load(file)
...
def generate_sql(data, sql_statements):
    ...
    VALUES ({data['BusinessTypeId']}, {data['ParentBusinessTypeId']}, '{data['BusinessTypeName']}', '{data['BusinessTypeCode']}', {order_num}, 6329);
    """
...
# 保存SQL语句到文件
with open('data/oa/08.sql', 'w', encoding='utf-8') as file:
    file.write("\n".join(sql_statements))
