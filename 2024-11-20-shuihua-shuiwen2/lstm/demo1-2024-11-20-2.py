...
X, Y = create_dataset(data, time_step, future_days=7)
...
# 保存模型
model.save(f'2024-11-20-shuihua-shuiwen2/model-lstm/lstm_relu_model_{current_time}.keras')
...
# 打印原始的14天数据
print("原始的14天数据：")
last_14_days_df = pd.DataFrame(last_14_days, columns=['气温（℃）', '相对湿度（%）', '降雨量（mm）', '风速（km/h）', '光照时长（h）'])
print(last_14_days_df)
...
# 保存预测结果到一个新的CSV文件
prediction_df.to_csv(f'2024-11-20-shuihua-shuiwen2/result/prediction_results_lstm_{current_time}.csv', index=False)
