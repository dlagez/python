...
mae_per_day_1 = results.groupby('Trddt').apply(lambda x: np.sqrt(mean_squared_error(x['y_true'], x['y_pred_1'])))
mae_per_day_2 = results.groupby('Trddt').apply(lambda x: np.sqrt(mean_squared_error(x['y_true'], x['y_pred_2'])))

print(mae_per_day_1.head())

# 创建折线图
plt.figure(figsize=(10, 6))
plt.plot(mae_per_day_1.index, mae_per_day_1.values, label='RandomForest MAE ()')
plt.plot(mae_per_day_2.index, mae_per_day_2.values, label='LinearRegression MAE (LinearRegression)', linestyle='--')
