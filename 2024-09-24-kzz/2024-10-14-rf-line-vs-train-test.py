...
# 绘制类别为 1 的数据的折线图
plt.plot(trddt[labels == 1], y_true[labels == 1], label='True Values (Class 1)', color='black', linestyle='-', marker='o', markersize=4)
plt.plot(trddt[labels == 1], y_pred_1[labels == 1], label='RandomForestRegressor Predictions (Class 1)', color='blue', linestyle='--', marker='x', markersize=4)
plt.plot(trddt[labels == 1], y_pred_2[labels == 1], label='LinearRegression Predictions (Class 1)', color='green', linestyle='-.', marker='s', markersize=4)

# 绘制类别为 2 的数据的折线图
plt.plot(trddt[labels == 2], y_true[labels == 2], label='True Values (Class 2)', color='gray', linestyle='-', marker='o', markersize=4)
plt.plot(trddt[labels == 2], y_pred_1[labels == 2], label='RandomForestRegressor Predictions (Class 2)', color='cyan', linestyle='--', marker='x', markersize=4)
plt.plot(trddt[labels == 2], y_pred_2[labels == 2], label='LinearRegression Predictions (Class 2)', color='lime', linestyle='-.', marker='s', markersize=4)
